    ...
    : describe("NFT Marketplace Tests", () => {
          ...
          describe("cancelListing", () => {
              ...
              it("reverts if anyone but the owner tries to call", async () => {
                  ...
                  basicNft.approve(player.address, TOKEN_ID)
                  await expect(
                      nftMarketplace.cancelListing(basicNft.address, TOKEN_ID)
                  ).to.be.revertedWith("NftMarketplace__NotOwner")
              })
              it("emits event and removes listing", async () => {
                  await nftMarketplace.listItem(basicNft.address, TOKEN_ID, PRICE)
                  expect(await nftMarketplace.cancelListing(basicNft.address, TOKEN_ID)).to.be.emit(
                      "ItemCanceled"
                  )
                  const listing = await nftMarketplace.getListing(basicNft.address, TOKEN_ID)
          ...
          describe("updateListing", () => {
              ...
              it("updates the price of the item", async () => {
                  ...
                  const listing = await nftMarketplace.getListing(basicNft.address, TOKEN_ID)
                  assert(updatedPrice.toString() == listing.price.toString())
              })
          })
